...
public static class HWPUtility {
    ...
    public static float RetineAspect {
        get {
            ...
            float from = Mathf.Log (vector.x / HWPManager.instance.m_ReferenceResolution.x, 2f);
            float to = Mathf.Log (vector.y / HWPManager.instance.m_ReferenceResolution.y, 2f);
            float p = Mathf.Lerp (from, to, HWPManager.instance.m_MatchWidthOrHeight);
