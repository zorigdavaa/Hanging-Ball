...
namespace SpriteParticleEmitter {
    ...
    public class UIParticleRenderer : MaskableGraphic {
        ...
        protected bool Initialize() {
            ...
            if (!pSystem) {
                pSystem = gameObject.Gc<ParticleSystem>();
                ...
                pRenderer = pSystem.Gc<ParticleSystemRenderer>();
                if (pRenderer)
        ...
        protected override void OnPopulateMesh(VertexHelper vh) {
            ...
            for (int i = 0; i < count; ++i) {
...
#if UNITY_5_5_OR_NEWER
                Vector2 position = (mainModule.simulationSpace == ParticleSystemSimulationSpace.Local ? particle.position : _transform.TfInvPnt(particle.position));
#else
                    Vector2 position = (pSystem.simulationSpace == ParticleSystemSimulationSpace.Local ? particle.position : _transform.TfInvPnt(particle.position));
#endif
                ...
                if (textureSheetAnimation.enabled) {
                    ...
                    frameProgress = M.Rep(frameProgress * textureSheetAnimation.cycleCount, 1);
                    int frame = 0;
        ...
        void Update() {
            if (!fixedTime && Application.isPlaying) {
                ...
                if ((currentMaterial && currentTexture != currentMaterial.mainTexture) ||
                    (material && currentMaterial && material.shader != currentMaterial.shader)) {
                    pSystem = null;
                    Initialize();
                }
            }
